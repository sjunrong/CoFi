...
import warnings
import pandas as pd
from data import load_adjM_type_mask, load_adjlists_idxs, load_normalized_adj,load_feature,Strategy_P
...
def train_epoch(net_fine, net_coarse, optimizer, train_pos_samples, train_neg_samples, features_list, adjlists, idxs, type_mask,  drug_feats, target_feats,disease_feats, drug_adjs, target_adjs, device, w1, w2,epoch):
    ...
    clip_norm = get_clip_norm(epoch, total_epochs=20, start_clip=3.5, end_clip=0.8)
...
def run():
    ...
    main_output_dir = os.path.join(current_dir, f"Seed_{args.seed}")
    ...
    folds = Strategy_P(seed=args.seed)
    drug_adj_paths = [
    ...
    for fold_idx, (train_pos, val_pos, test_pos, train_neg, val_neg, test_neg) in enumerate(zip(*folds)):
        ...
        print(f"Test Results for Fold {fold_idx}: {test_results}")

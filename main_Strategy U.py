...
import sys
import pandas as pd
from data import load_adjM_type_mask, load_adjlists_idxs, load_normalized_adj,load_feature,Strategy_U
from loss import n_pair_loss,contrastive_loss
from eval import evaluate_unbiased, compute_precision_aupr
...
def train_epoch(net_fine, net_coarse, optimizer, train_pos_samples, train_neg_samples, features_list, adjlists, idxs, type_mask,  drug_feats, target_feats,disease_feats, drug_adjs, target_adjs, device, w1, w2,epoch):
    ...
    n_loss_list = []
    compare_loss_list = []
    all_beta_user, all_beta_item = [], []  # 新增，用于保存训练权重
    ...
    clip_norm = get_clip_norm(epoch, total_epochs=60, start_clip=6.0, end_clip=1.0)
    # clip_norm = get_clip_norm(epoch, total_epochs=30, start_clip=3.5, end_clip=0.8)
    for iteration in range(train_pos_idx_generator.num_iterations()):
        ...
        torch.nn.utils.clip_grad_norm_(
            ...
            max_norm= clip_norm,  # 建议初始阈值 0.6 ratio
            norm_type=2    # L2范数
        )
        optimizer.step()
...
def validate_epoch(net_fine, net_coarse,val_pos_samples, val_neg_samples, features_list, adjlists, idxs, type_mask, drug_feats, target_feats, disease_feats,drug_adjs, target_adjs,device,w1,w2):
    ...
    all_pos_scores = []
    all_neg_scores = []
    val_pos_idx_generator = index_generator(num_batches=1, num_data=len(val_pos_samples),shuffle=True)
...
def test_model(net_fine,net_coarse, test_pos_samples, test_neg_samples, features_list, adjlists, idxs, type_mask,drug_feats, target_feats, disease_feats,drug_adjs, target_adjs, device,fold_dir):
    ...
    return evaluate_unbiased(all_pos_scores, all_neg_scores)
...
def run():
    ...
    main_output_dir = os.path.join(current_dir,f"Seed_u{args.seed}" )
    ...
    folds = Strategy_U(seed=args.seed)
    drug_adj_paths = [
    ...
    for fold_idx, (train_pos, val_pos, test_pos, train_neg, val_neg, test_neg) in enumerate(zip(*folds)):
